    ...
    var persianDatepicker = (function() {
        ...
        persianDatepicker.prototype = {
            ...
            header: function() {
                ...
                var _monthSelect = $('<ul class="monthSelect" ' + self.selectUlStyle + ' />');
                var _yearSelect = $('<ul class="yearSelect" ' + self.selectUlStyle + ' />');
                // Build month label
                var _monthText = $('<div/>').html(self.options.months[self.persianDate.month - 1]).mousedown(function() {
                    return false;
                }).click(function(e) {
                    e.stopPropagation();
                    _yearSelect.removeClass('show');
                    _monthSelect.addClass('show');
                });
                var _yearText = $('<div/>').html(self.options.persianNumbers ? self.jDateFunctions.toPersianNums(self.persianDate.year) : self.persianDate.year).mousedown(function() {
                    return false;
                }).click(function(e) {
                    e.stopPropagation();
                    _monthSelect.removeClass('show');
                    _yearSelect.addClass('show');
                ...
                var titleYearMonth = $('<div class="monthYear" />').append(_monthText).append(_yearText);
            ...
            footer: function() {
                ...
                if (self.options.selectableMonths._indexOf(self.persianDate.month) > -1) {
                    ...
                    _goToday.attr("href", "javascript:;").html(self.options.goTodayText);
                    if (self.options.startDate == null)
